...
<?= $this->section('content') ?>
      ...
      <div class="bg-white rounded-xl shadow-md overflow-hidden max-w-full mx-auto" x-data="pageList()">
        <div class="p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <h1 class="text-xl md:text-2xl font-semibold text-gray-800"><?= lang('validation.service_manage') ?></h1>
          <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
            ...
            <a href="<?= site_url('admin/services/create') ?>" class="flex items-center justify-center bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium py-2 px-4 rounded-lg shadow-sm">
               <i class="fas fa-user-plus mr-2"></i> <?= lang('validation.add_service') ?>
            </a>
          </div>
        </div>
          ...
          <table class="w-full divide-y divide-gray-200">
            ...
            <?php foreach ($services as $service): ?>
              <tr class="hover:bg-gray-50">
                <td class="p-3 w-4"><input type="checkbox" class="rounded border-gray-300 text-blue-600" value="<?= $service['id'] ?>"></td>
                  <td class="px-4 py-2 whitespace-nowrap">
                <img src="<?= $service['thumbnail'] ?? ('/placeholder-image.jpg') ?>" alt="<?= esc($service['name']) ?>" class="h-10 w-10 rounded-md object-cover border border-gray-200">
              </td>

                <td class="px-4 py-2 text-sm font-medium text-gray-900 whitespace-normal break-words"><?= esc($service['name']) ?></td>
               <td class="px-4 py-2 text-sm text-gray-500 whitespace-normal break-words hidden sm:table-cell"><?= esc($service['description']) ?></td>
                ...
                <td class="px-4 py-2 text-right text-sm font-medium space-x-2">
                  <a href="<?= site_url('admin/services/edit/' . $service['id']) ?>" class="text-blue-600 hover:text-blue-800"><i class="fas fa-edit"></i></a>
                <!--   <?= site_url('admin/services/delete/' . $service['id']) ?> -->
