...
<div x-data="newsFormData()" x-init="featuredImageUrl = '<?= $partner['logo'] ?? '' ?>'" @select-image.window="handleImageSelection($event.detail)">
  <div class="bg-white rounded-xl shadow-md overflow-hidden max-w-xl mx-auto mt-8">
    <div class="p-6 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <h1 class="text-xl md:text-2xl font-semibold text-gray-800">Chỉnh sửa Partner</h1>
    ...
    <div class="p-6">
      <form action="/admin/partners/update/<?= $partner['id'] ?>" method="POST" enctype="multipart/form-data" class="space-y-5">
        <div>
          <label class="block font-semibold mb-1">Tên</label>
          <input type="text" name="name" value="<?= $partner['name'] ?>" class="form-input w-full rounded border-gray-300 focus:ring-blue-500" required>
        </div>
        <div>
          <label class="block font-semibold mb-1">Tên Tiếng Anh</label>
          <input type="text" name="name_en" value="<?= $partner['name_en'] ?>" class="form-input w-full rounded border-gray-300 focus:ring-blue-500" required>
        </div>
        ...
        <div>
          ...
          <select name="status" class="form-select w-full rounded border-gray-300 focus:ring-blue-500">
            <option value="1" <?= $partner['status'] ? 'selected' : '' ?>>Hoạt động</option>
            <option value="0" <?= !$partner['status'] ? 'selected' : '' ?>>Không hoạt động</option>
          </select>
        </div>
        <div class="pt-2">
