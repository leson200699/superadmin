...
<main class="flex-1 p-4 md:p-6 overflow-auto" x-data="{
  ...
  deleteUrl: '',
  triggerDelete(id) {
    this.deleteUrl = '/admin/product/delete/id/' + id;
  ...
  <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
    ...
    <div class="flex gap-2 items-center">
      ...
      <a href="/admin/product/product-create" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 text-sm rounded-md shadow-sm">
        <i class="fas fa-plus mr-2"></i> <?= lang('validation.add_product') ?>
      </a>
    </div>
  </div>
  ...
  <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
    ...
    <div class="sm:col-span-2 lg:col-span-4">
      <p class="text-sm text-gray-500 mb-4">
        Hiển thị <?= $pager['from'] ?>-<?= $pager['to'] ?> trong tổng số <?= $pager['total'] ?> sản phẩm
      </p>
    </div>
    <?php endif; ?>
    ...
    <div class="bg-white rounded-xl shadow flex flex-col h-full overflow-hidden" 
      ...
      <div class="w-[93%] mx-auto mt-2 relative">
        <!-- Lớp phủ mờ xám chỉ cho phần thumbnail khi sản phẩm bị tắt -->
  ...
  <div class="flex flex-col justify-between flex-grow p-4 space-y-3">
    <div class="space-y-1">
      <p class="text-sm font-semibold text-gray-400 line-clamp-3"><?= number_format($item->price) ?> đ</p>
      <h2 class="text-base font-semibold text-gray-800 line-clamp-2"><?= esc($item->name) ?></h2>
      <p class="text-sm text-gray-500 line-clamp-3"><?= esc($item->caption) ?></p>
    ...
    <div class="flex items-center justify-between pt-2 border-t border-gray-100">
      ...
      <div class="flex items-center">
        <button
          type="button"
          @click="toggleProductStatus(<?= $item->id ?>, disabled)"
      ...
      <div class="flex items-center gap-1">
        <a href="<?= route_to('admin-product-edit', $item->id) ?>" class="px-3 py-1.5 rounded-md bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs">
          <i class="fas fa-pencil-alt mr-1"></i><?= lang('validation.edit') ?>
        </a>
        <button @click="triggerDelete(<?= $item->id ?>)" class="px-3 py-1.5 rounded-md bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs">
          <i class="fas fa-trash-alt mr-1"></i><?= lang('validation.delete') ?>
        </button>
      </div>
    </div>
  </div>
