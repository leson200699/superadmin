...
<?= $this->section('content') ?>
 <div class="bg-white rounded-xl shadow-md overflow-hidden max-w-full mx-auto" x-data="{
        ...
        deleteUrl: '',
        triggerDelete(id) {
          this.deleteUrl = '/admin/news/delete-category/' + id;
        ...
        <div class="p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
            ...
            <a href="<?= site_url('admin/news/category-create') ?>" class="flex items-center justify-center bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium py-2 px-4 rounded-lg shadow-sm">
          ...
          <table class="w-full divide-y divide-gray-200">
            ...
            <?php foreach ($category_list as $item): ?>
              <tr class="hover:bg-gray-50">
                ...
                <img src="<?= $item['thumbnail'] ?? ('/placeholder-image.jpg') ?>" alt="<?= esc($item['name']) ?>" class="h-10 w-10 rounded-md object-cover border border-gray-200">
              </td>


                <td class="px-4 py-2 text-sm font-medium text-gray-900 whitespace-normal break-words"><?= esc($item['name']);?></td>
               <td class="px-4 py-2 text-sm text-gray-500 whitespace-normal break-words hidden sm:table-cell"><?= esc($item['parent_name']);?></td>
                ...
                <td class="px-4 py-2 text-right text-lg font-medium space-x-2">
                  ...
                  <a href="<?= route_to('admin-news-category-edit', $item['id']) ?>" class="text-gray-500 hover:text-green-600"><i class="fa-solid fa-pen-circle"></i></a>
