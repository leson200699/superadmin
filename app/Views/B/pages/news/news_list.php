...
<main class="flex-1 p-4 md:p-6 overflow-auto" x-data="{
  ...
  deleteUrl: '',
  triggerDelete(id) {
    this.deleteUrl = '/admin/news/delete/id/' + id;
  ...
  <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
    ...
    <div class="flex gap-2 items-center">
      ...
      <a href="/admin/news/news-create" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 text-sm rounded-md shadow-sm">
  ...
  <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
    ...
    <div class="sm:col-span-2 lg:col-span-4">
      <p class="text-sm text-gray-500 mb-4">
        Hiển thị <?= $pager['from'] ?>-<?= $pager['to'] ?> trong tổng số <?= $pager['total'] ?> tin tức
      </p>
    </div>
    <?php endif; ?>

    <?php foreach ($news_list as $item): ?>
    <div class="bg-white rounded-xl shadow flex flex-col h-full overflow-hidden relative" 
      x-data="{ 
        disabled: <?= $item->status == 0 ? 'true' : 'false' ?>,
        toggleNewsStatus(id, newStatus) {
          fetch('/admin/news/status', {
            method: 'POST',
            headers: {
              'X-Requested-With': 'XMLHttpRequest',
              'Content-Type': 'application/x-www-form-urlencoded'
      ...
      <div class="w-[93%] mx-auto mt-2 relative">
        <!-- Lớp phủ mờ xám chỉ cho phần thumbnail khi bài viết bị tắt -->
  ...
  <div class="flex flex-col justify-between flex-grow p-4 space-y-3">
    ...
    <div class="flex items-center justify-between pt-2 border-t border-gray-100">
      ...
      <div class="flex items-center">
        <button
          type="button"
          @click="toggleNewsStatus(<?= $item->id ?>, disabled)"
      ...
      <div class="flex items-center gap-1">
        <a href="<?= route_to('admin-news-edit', $item->id) ?>" class="px-3 py-1.5 rounded-md bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs">
