...
<div x-data="newsFormData()" x-init="featuredImageUrl = '<?= esc($category['thumbnail'] ?? '') ?>'" @select-image.window="handleImageSelection($event.detail)">
    <h1 class="text-xl md:text-2xl font-semibold text-gray-800 mb-6">
        Chỉnh sửa danh mục dịch vụ
    </h1>
    <form action="/admin/services/category/update/<?= $category['id'] ?>" method="post" enctype="multipart/form-data" class="space-y-6">
        <?= csrf_field() ?>
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white p-5 rounded-lg shadow space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" required class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4 text-base" placeholder="Nhập tên danh mục..." value="<?= esc($category['name']) ?>">
                    </div>
                    <div>
                        ...
                        <select name="parent_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4 text-base">
                            ...
                            <?php foreach ($parentCategories as $parent): ?>
                                <option value="<?= $parent['id']; ?>" <?= (isset($category['parent_id']) && $parent['id'] == $category['parent_id']) ? 'selected' : '' ?>><?= esc($parent['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
                        ...
                        <select class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4 text-base" name="status" required>
                            <option value="1" <?= isset($category['status']) && $category['status'] == 1 ? 'selected' : '' ?>>Kích hoạt</option>
                            <option value="0" <?= isset($category['status']) && $category['status'] == 0 ? 'selected' : '' ?>>Tạm dừng</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Tiêu đề</label>
                        <input type="text" name="title" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4 text-base" placeholder="Nhập tiêu đề danh mục" value="<?= esc($category['title'] ?? '') ?>">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Từ khóa</label>
                        <input type="text" name="keyword" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4 text-base" placeholder="Nhập từ khóa" value="<?= esc($category['keyword'] ?? '') ?>">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Mô tả</label>
                        <textarea name="description" rows="3" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 py-3 px-4 text-base" placeholder="Nhập mô tả danh mục"><?= esc($category['description'] ?? '') ?></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <a href="/admin/service-category-list" class="px-5 py-2.5 text-sm font-medium ml-2 inline-flex items-center text-gray-700 bg-gray-200 hover:bg-gray-300 rounded-lg">
