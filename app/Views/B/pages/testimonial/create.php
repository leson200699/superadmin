...
<div x-data="newsFormData()" @select-image.window="handleImageSelection($event.detail)">
  <div class="bg-white rounded-xl shadow-md overflow-hidden max-w-xl mx-auto mt-8">
    <div class="p-6 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <h1 class="text-xl md:text-2xl font-semibold text-gray-800">Thêm đánh giá khách hàng</h1>
    ...
    <div class="p-6">
      <form action="/admin/testimonial/store" method="post" enctype="multipart/form-data" class="space-y-5">
        <?= csrf_field() ?>
        <div>
          <label class="block font-semibold mb-1">Tên khách hàng</label>
          <input type="text" name="customer_name" class="form-input w-full rounded border-gray-300 focus:ring-blue-500" required>
        </div>
        <div>
          <label class="block font-semibold mb-1">Nội dung đánh giá</label>
          <textarea name="testimonial" class="form-textarea w-full rounded border-gray-300 focus:ring-blue-500" rows="3" required></textarea>
        </div>
        <div>
          <label class="block font-semibold mb-1">Nghề nghiệp</label>
          <input type="text" name="career" class="form-input w-full rounded border-gray-300 focus:ring-blue-500" required>
        </div>
        ...
        <div class="pt-2">
          <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-6 rounded-lg shadow">Lưu đánh giá</button>
        </div>
      </form>
    </div>
  </div>
