...
<div class="bg-white rounded-xl shadow-md overflow-hidden max-w-full mx-auto" x-data="{
        ...
        deleteUrl: '',
        triggerDelete(id) {
          this.deleteUrl = '/admin/testimonial/delete/' + id;
  ...
  <div class="p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <h1 class="text-xl md:text-2xl font-semibold text-gray-800">Danh sách đánh giá khách hàng</h1>
    <a href="/admin/testimonial/create" class="flex items-center justify-center bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium py-2 px-4 rounded-lg shadow-sm">
    ...
    <table class="w-full divide-y divide-gray-200">
      <thead class="bg-gray-50">
        <tr>
          ...
          <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nghề nghiệp</th>
      ...
      <tbody class="bg-white divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-50">
          <td class="p-3 w-4"><input type="checkbox" class="rounded border-gray-300 text-blue-600" value="<?= $testimonial['id'];?>"></td>
          <td class="px-4 py-2 text-sm text-gray-900 whitespace-nowrap"><?= $testimonial['id'] ?></td>
          <td class="px-4 py-2 text-sm text-gray-900 whitespace-nowrap"><?= esc($testimonial['customer_name']) ?></td>
          <td class="px-4 py-2 text-sm text-gray-700 whitespace-normal break-words"><?= esc($testimonial['testimonial']) ?></td>
          <td class="px-4 py-2 text-sm text-gray-700 whitespace-nowrap"><?= esc($testimonial['career']) ?></td>
          <td class="px-4 py-2 whitespace-nowrap">
            <?php if ($testimonial['thumbnail']): ?>
              <img src="<?= $testimonial['thumbnail'] ?>" alt="Customer Image" class="h-10 w-10 rounded-md object-cover border border-gray-200">
            <?php else: ?>
              <span class="text-gray-400 italic">No Image</span>
            <?php endif; ?>
          </td>
          <td class="px-4 py-2 text-right text-lg font-medium space-x-2">
            <a href="/admin/testimonial/edit/<?= $testimonial['id'] ?>" class="text-gray-500 hover:text-green-600"><i class="fa-solid fa-pen-circle"></i></a>
            <button @click="triggerDelete(<?= $testimonial['id'] ?>)" class="text-red-600 hover:text-red-800"><i class="fa-solid fa-circle-minus"></i></button>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
  ...
  <div x-show="confirmDelete" x-cloak class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-sm p-6">
      ...
      <p class="text-sm text-gray-600 mb-6">Bạn có chắc chắn muốn xoá đánh giá này không? Hành động này không thể hoàn tác.</p>
  ...
  <div class="p-4 border-t border-gray-200 flex items-center justify-between">
    <span class="text-sm text-gray-600">Hiển thị <?= count($testimonials) ?> đánh giá</span>
