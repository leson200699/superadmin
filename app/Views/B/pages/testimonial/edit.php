...
<div x-data="newsFormData()" x-init="featuredImageUrl = '<?= $testimonial['thumbnail'] ?>'" @select-image.window="handleImageSelection($event.detail)">
  <div class="bg-white rounded-xl shadow-md overflow-hidden max-w-xl mx-auto mt-8">
    <div class="p-6 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <h1 class="text-xl md:text-2xl font-semibold text-gray-800">Chỉnh sửa đánh giá khách hàng</h1>
      <a href="/admin/testimonial" class="flex items-center justify-center bg-gray-200 hover:bg-gray-300 text-gray-800 text-sm font-medium py-2 px-4 rounded-lg shadow-sm">
        <i class="fas fa-arrow-left mr-2"></i> Quay lại
      </a>
    </div>
    <div class="p-6">
      <form action="/admin/testimonial/update/<?= $testimonial['id'] ?>" method="post" enctype="multipart/form-data" class="space-y-5">
        <?= csrf_field() ?>
        <div>
          <label class="block font-semibold mb-1">Tên khách hàng</label>
          <input type="text" name="customer_name" value="<?= $testimonial['customer_name'] ?>" class="form-input w-full rounded border-gray-300 focus:ring-blue-500" required>
        </div>
        <div>
          <label class="block font-semibold mb-1">Nội dung đánh giá</label>
          <textarea name="testimonial" class="form-textarea w-full rounded border-gray-300 focus:ring-blue-500" rows="3" required><?= $testimonial['testimonial'] ?></textarea>
        </div>
        <div>
          <label class="block font-semibold mb-1">Nghề nghiệp</label>
          <input type="text" name="career" value="<?= $testimonial['career'] ?>" class="form-input w-full rounded border-gray-300 focus:ring-blue-500" required>
        </div>
        ...
        <div class="pt-2">
          <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-6 rounded-lg shadow">Cập nhật đánh giá</button>
        </div>
      </form>
    </div>
  </div>
