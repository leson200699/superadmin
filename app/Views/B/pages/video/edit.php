...
<div x-data="newsFormData()" x-init="videoUrl = '<?= esc($video['file_path']) ?>'; thumbUrl = '<?= esc($video['thumbnail']) ?>'" @select-image.window="handleVideoSelection($event.detail)">
  <div class="bg-white rounded-xl shadow-md overflow-hidden max-w-xl mx-auto mt-8">
    <div class="p-6 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <h1 class="text-xl md:text-2xl font-semibold text-gray-800">Chỉnh sửa Video</h1>
    ...
    <div class="p-6">
      ...
      <?php endif; ?>
      <form action="<?= site_url('admin/video/update/' . $video['id']) ?>" method="post" class="space-y-5">
        <?= csrf_field() ?>
        <div>
          <label class="block font-semibold mb-1">Tiêu đề</label>
          <input type="text" name="title" class="form-input w-full rounded border-gray-300 focus:ring-blue-500" value="<?= esc($video['title']) ?>" required>
        </div>
        <div>
          <label class="block font-semibold mb-1">Mô tả</label>
          <textarea name="description" class="form-textarea w-full rounded border-gray-300 focus:ring-blue-500" rows="3"><?= esc($video['description']) ?></textarea>
        </div>
        ...
        <div>
          ...
          <input type="number" name="duration" class="form-input w-full rounded border-gray-300 focus:ring-blue-500" value="<?= esc($video['duration']) ?>">
        </div>
        <div class="pt-2">
          <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-6 rounded-lg shadow">Cập nhật</button>
        </div>
      </form>
    </div>
  </div>
